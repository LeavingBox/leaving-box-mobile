...
- `getSession` : polling d’état salle d’attente (agent/waitingRoom) et vérif code côté opérateur (`app/analyst/joinGame.tsx`).
...
- `gameStarted` : payload `{ moduleManuals, solutionsByAnalyst }`; côté opérateur, on filtre avec `Socket.id` et on fusionne les solutions avant d’ouvrir l’écran manuel.
